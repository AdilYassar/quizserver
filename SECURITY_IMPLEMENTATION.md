...
- `/student/register` - Student registration with validation
- `/admin/register` - Admin registration with validation
...
- `/user/enrollment-stats` - Enrollment statistics
- `/user/password` - Password change
...
A comprehensive test suite is provided in `test-secure-auth.js` that covers:
...
1. **Update User Model**: Ensure all users have UUIDs
2. **Update Controllers**: Replace userId references with userUuid
3. **Update Middleware**: Use new authentication middleware
4. **Update Routes**: Apply new validation and rate limiting
5. **Test Thoroughly**: Run the security test suite
